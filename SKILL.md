...
description: 用于生成“美股见顶 / 崩盘监控报告”的可执行决策系统。Use when an AI agent or analyst needs to monitor US equity top/crash risk, judge whether the market is in a fragile high-zone, detect risk transmission into credit/rates/funding, assess breadth and leadership concentration, scan event overrides, and map conclusions to green/yellow/orange/red portfolio actions.
---
...
- 是否存在会迅速改变市场状态的重大事件。
- 当前应映射成什么操作级别：绿 / 黄 / 橙 / 红。

这不是泛泛市场评论。必须从监控走到动作建议。
...
- 动作：停止新增高 beta；新信号减半；保留更多现金；不主动追纳指 / 日股；观望标的提高准入门槛。

橙灯：

- 特征：两个独立模块同时恶化；或事件层已明显冲击风险偏好；或慢变量压力与结构恶化叠加。
- 动作：停止所有高 beta 加仓；防守资产优先；检查沉淀层是否需要对冲思维；观望标的不纳入；新仓只允许极少数高把握、低相关方向。
